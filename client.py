...
class HNSWVectorIndexClient:
    ...
    def add_vector(self, node_id: str, vector: List[float]):
        ...
        for l in range(len(self.layers) - 1, node_level, -1):
            curr_ep = self._search_layer_greedy(curr_ep, vector, l)

        # Connect at layers <= node_level
        for l in range(min(node_level, len(self.layers) - 1), -1, -1):
            self.layers[l][node_id] = []
        ...
        if node_level > len(self.layers) - 1:
            self.entry_point = node_id
